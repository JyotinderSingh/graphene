import { IGraph, verticesType, vertexType, edgesType, vertexIndexType, edgeType } from "./types/primitives";
  ...
  export class Graph implements IGraph {
    ...
    addVertex = (vertex: vertexType): number | boolean => {
      if (!vertex._id) {
        ...
        return Graphene.error("A vertex with that ID already exists.");
      }
    ...
    addEdge = (edge: edgeType): void | boolean => {
      ...
      if (!(edge._in && edge._out)) {
        return Graphene.error("That edge's " + (edge._in ? 'out' : 'in')
          + " vertex wasn't found")
      }
